  ...
  return (
    <aside className="w-64 bg-white dark:bg-slate-900/90 border-r border-slate-200 dark:border-slate-800 flex flex-col justify-between min-h-[calc(100vh-61px)] p-4 select-none shrink-0 hidden md:flex">
                ...
                <div key={item.id} className="space-y-1">
                  <button
                    ...
                    className={`w-full flex items-center justify-between px-3 py-2.5 rounded-xl font-medium text-xs transition-all group ${
                      isActive 
                        ? 'bg-blue-50 dark:bg-blue-600/15 text-[#0062bd] dark:text-blue-400 border border-blue-200 dark:border-blue-500/30 font-bold shadow-sm' 
                  ...
                  >
                    <div className="flex items-center space-x-3">
                      <Icon className={`w-4 h-4 transition-colors ${isActive ? 'text-[#0062bd] dark:text-blue-400' : 'text-slate-500 group-hover:text-slate-700'}`} />
                      <span>{item.label}</span>
                    </div>

                    {item.badge && (
                      <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-blue-100 dark:bg-blue-500/20 text-[#0062bd] dark:text-blue-300 border border-blue-200">
                        {item.badge}
                      </span>
                    )}

                    {hasSub && (
                      <ChevronRight className={`w-3.5 h-3.5 transition-transform text-slate-400 ${isActive ? 'rotate-90 text-[#0062bd]' : ''}`} />
                    )}
                  </button>
                        ...
                        return (
                          <button
                            ...
                            className={`w-full text-left px-3 py-1.5 rounded-lg text-[11px] transition-colors ${
                              isSubActive 
                                ? 'bg-blue-100 dark:bg-blue-500/20 text-[#0062bd] dark:text-blue-300 font-bold' 
      ...
      <div className="p-3 bg-slate-50 dark:bg-slate-800/40 rounded-xl border border-slate-200 dark:border-slate-800 text-xs text-slate-500">
        ...
        <p className="text-[11px] text-slate-400">2026 © srkinnovations.com</p>
      </div>
    </aside>
  );
};
